...
void nHCal_VM_Analysis(int RecChaPar=1, int mode=1, TString strang = "sartre_bnonsat_Au_phi_ab_eAu_q2_15_1_1run"){
  ...
  if(mode==1) // streaming runlist (default)
    {
      runlist = TString("runlists/") + strang + TString("_runlist.txt"); 
    }
  if(mode==2) // local runlist (reading locally stored files)
    {
      runlist = TString("local_runlists/") + strang + TString("_runlist.txt");
    }
 ...
 // LFHCal clusters: fParentName edm4eic::ClusterData
  ...
  //// muons
  TH1D *nHCalClustersnHits_muons = new TH1D("nHCalClustersnHits_muons","nHits of muon clusters in the nHCal; number of hits",16,0,15);
  ...
  //// charged kaons
  TH1D *nHCalClustersnHits_kaons = new TH1D("nHCalClustersnHits_kaons","nHits of kaon clusters in the nHCal; number of hits",16,0,15);
  ...
  //// electrons (+-)
  TH1D *nHCalClustersnHits_electrons = new TH1D("nHCalClustersnHits_electrons","nHits of electron clusters in the nHCal; number of hits",16,0,15);
  ...
  TH2D *nHCalRecHitsPosXY_all = new TH2D("nHCalRecHitsPosXY_all","nHCalRecHits_all XY; nHCalRecHits.position.x [mm]; nHCalRecHits.position.y [mm]", 1000,hx_min_nhcal,hx_max_nhcal,1000,hy_min_nhcal,hy_max_nhcal);
         ...
         cout << " MCParticle: " << i << ", PDG: " << partPdg[i] << ", matching cluster ID in the nHCal: " << k << ", cluster energy: " << nHCalClustersE[recoAssocClusters_nHCal[k]] << ", cluster position X: " << nHCalClustersPosX[recoAssocClusters_nHCal[k]] <<  ", cluster position Y: " << nHCalClustersPosY[recoAssocClusters_nHCal[k]] <<  ", cluster position Z: " << nHCalClustersPosZ[recoAssocClusters_nHCal[k]] << ", hits begin: " << nHCalClustershits_begin[recoAssocClusters_nHCal[k]] << ", hits end: " << nHCalClustershits_end[recoAssocClusters_nHCal[k]] << " \n";
            ...
            for(unsigned int h=nHCalClustershits_begin[recoAssocClusters_nHCal[k]]; h<=nHCalClustershits_end[recoAssocClusters_nHCal[k]]; h++){
                ...
                for(int l=0; l<nlayers_nhcal; l++){
                    // min is larger than max and both are in mm
            ...
            if( pdg == 13){
                nHCalClustersnHits_muons->Fill(nHCalClustersnHits[recoAssocClusters_nHCal[k]]);
                ...
                nHCalClustersnHits_kaons->Fill(nHCalClustersnHits[recoAssocClusters_nHCal[k]]);
                ...
                nHCalClustersnHits_electrons->Fill(nHCalClustersnHits[recoAssocClusters_nHCal[k]]);
                ...
                nHCalClustersPosXYZ_electrons->Fill(nHCalClustersPosX[recoAssocClusters_nHCal[k]],nHCalClustersPosY[recoAssocClusters_nHCal[k]],nHCalClustersPosZ[recoAssocClusters_nHCal[k]]);} // end e+-
